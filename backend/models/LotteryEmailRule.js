...
class LotteryEmailRule {
    ...
    static async create(ruleData) {
        const { email_account_id, report_type, to_address, subject_contains, sender_contains = 'palottery.com', retailer_number, is_active = true } = ruleData;
        
        const result = await query(
            `INSERT INTO lottery_email_rules (email_account_id, report_type, to_address, subject_contains, sender_contains, retailer_number, is_active)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING *`,
            [email_account_id, report_type, to_address, subject_contains, sender_contains, retailer_number, is_active]
        );
    ...
    static async update(id, updateData) {
        const { to_address, subject_contains, sender_contains, retailer_number, is_active } = updateData;
        const updates = [];
        const values = [];
        let paramCount = 1;
