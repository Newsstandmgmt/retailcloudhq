...
const OtherIncome = () => {
  ...
  const loadCategories = async () => {
    ...
    try {
      setLoading(true);
      const response = await settingsAPI.getOtherIncomeCategories(selectedStore.id);
      setCategories(response.data.categories || []);
    } catch (error) {
      console.error('Error loading other income categories:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddCategory = async (e) => {
    e.preventDefault();
    if (!selectedStore || !newCategory.trim()) return;

    try {
      await settingsAPI.createOtherIncomeCategory(selectedStore.id, { category_name: newCategory.trim() });
      setNewCategory('');
      loadCategories();
    } catch (error) {
  ...
  const handleEditCategory = async (categoryId) => {
    ...
    try {
      await settingsAPI.updateOtherIncomeCategory(categoryId, { category_name: editName.trim() });
      setEditingId(null);
      setEditName('');
      loadCategories();
    } catch (error) {
  ...
  const handleDeleteCategory = async (categoryId) => {
    ...
    try {
      await settingsAPI.deleteOtherIncomeCategory(categoryId);
      loadCategories();
    } catch (error) {
    ...
    <div className="p-6 w-full min-w-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 w-full">
        {categories.map((category) => (
          <div key={category.id} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-2">
            {editingId === category.id ? (
              ...
              <>
                <span className="flex-1 text-sm font-medium text-gray-900">{category.category_name}</span>
                <button
                  onClick={() => {
      ...
      {categories.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500">No categories added yet. Add a category above to get started.</p>
        </div>
      )}
