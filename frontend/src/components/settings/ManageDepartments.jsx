...
const ManageDepartments = () => {
  ...
  const loadDepartments = async () => {
    ...
    try {
      setLoading(true);
      const response = await purchaseInvoicesAPI.getDepartments(selectedStore.id);
  ...
  const handleAddDepartment = async (e) => {
    e.preventDefault();
    if (!selectedStore || !newDepartment.trim()) return;

    try {
      await purchaseInvoicesAPI.createDepartment(selectedStore.id, { name: newDepartment.trim() });
      setNewDepartment('');
      loadDepartments();
    } catch (error) {
      alert('Error adding department: ' + (error.response?.data?.error || error.message));
    }
  };
  ...
  const handleEditDepartment = async (deptId) => {
    ...
    try {
      await purchaseInvoicesAPI.updateDepartment(deptId, { name: editName.trim() });
      setEditingId(null);
      setEditName('');
      loadDepartments();
    } catch (error) {
  ...
  const handleDeleteDepartment = async (deptId) => {
    ...
    try {
      await purchaseInvoicesAPI.deleteDepartment(deptId);
      loadDepartments();
    } catch (error) {
    ...
    <div className="p-6 w-full min-w-0">
      ...
      <div className="mb-6 flex items-center gap-2">
        <input
          ...
          onKeyPress={(e) => e.key === 'Enter' && handleAddDepartment(e)}
        />
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 w-full">
        {departments.map((dept) => (
          <div key={dept.id} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-2">
            {editingId === dept.id ? (
              <>
                <input
                  type="text"
                  ...
                  onBlur={() => handleEditDepartment(dept.id)}
                />
                <button
                  onClick={() => {
                    setEditingId(null);
      ...
      {departments.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500">No departments added yet. Add a department above to get started.</p>
        </div>
      )}
